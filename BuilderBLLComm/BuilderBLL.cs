...
namespace Maticsoft.BuilderBLLComm
{
    ...
    public class BuilderBLL : IBuilder.IBuilderBLL
    {
        ...
        public string GetBLLCode(bool Maxid, bool Exists, bool Add, bool Update, bool Delete, bool GetModel, bool GetModelByCache, bool List)
        {
            ...
            {
                //strclass.AppendLine("");
                strclass.AppendLine(string.Format("using {0}.Common;", NameSpace));
            }
        ...
        public string CreatBLLADD()
        {
            ...
            StringPlus strclass0 = new StringPlus();// 定义变量
            StringPlus strclass1 = new StringPlus();// 数据验证 ，变量赋值
            StringPlus strclass2 = new StringPlus();// model 赋值
            ...
            foreach (ColumnInfo field in Fieldlist)
            {
                ...
                if (field.Nullable)
                {
                    switch (CodeCommon.DbTypeToCS(columnType.Trim().ToLower()).ToLower())
                    {
                        case "int":
                        case "smallint":
                            if (DbType == "Oracle") strclass1.AppendSpaceLine(3, "if(!string.IsNullOrEmpty(" + columnNameToLower + ")&&!PageValidate.IsNumber(" + columnNameToLower + ")) strErr=\"" + deText + " 不是数字！\\\\n\"; if(!string.IsNullOrEmpty(strErr))return strErr;");
                            strclass2.AppendSpaceLine(3, "model." + columnName + "=StringHelper.Str2Int(" + columnNameToLower + ");");
                            break;
                        case "float":
                        case "numeric":
                        case "decimal":
                            if (DbType == "Oracle") strclass1.AppendSpaceLine(3, "if(!string.IsNullOrEmpty(" + columnNameToLower + ")&&!PageValidate.IsDecimal(" + columnNameToLower + ")) strErr=\"" + deText + " 不是数字！\\\\n\"; if(!string.IsNullOrEmpty(strErr))return strErr;");
                            strclass2.AppendSpaceLine(3, "model." + columnName + "=StringHelper.Str2Dec(" + columnNameToLower + ");");
                            break;
                        case "datetime":
                        case "smalldatetime":
                            if (DbType == "Oracle") strclass1.AppendSpaceLine(3, "if(!string.IsNullOrEmpty(" + columnNameToLower + ")&&!string.IsNullOrEmpty(" + columnNameToLower + ")&&!PageValidate.IsDateTime(" + columnNameToLower + ")) strErr=\"" + deText + " 不是时间字符串！\\\\n\"; if(!string.IsNullOrEmpty(strErr))return strErr;");
                ...
                else
                {
                    switch (CodeCommon.DbTypeToCS(columnType.Trim().ToLower()).ToLower())
                    {
                        case "int":
                        case "smallint":
                            if (DbType == "Oracle") strclass1.AppendSpaceLine(3, "if(!PageValidate.IsNumber(" + columnNameToLower + ")) strErr=\"" + deText + " 不是数字！\\\\n\"; if(!string.IsNullOrEmpty(strErr))return strErr;");
                            strclass2.AppendSpaceLine(3, "model." + columnName + "=int.Parse(" + columnNameToLower + ");");
                            break;
                        case "float":
                        case "numeric":
                        case "decimal":
                            if (DbType == "Oracle") strclass1.AppendSpaceLine(3, "if(!PageValidate.IsDecimal(" + columnNameToLower + ")) strErr=\"" + deText + " 不是数字！\\\\n\"; if(!string.IsNullOrEmpty(strErr))return strErr;");
                            strclass2.AppendSpaceLine(3, "model." + columnName + "=decimal.Parse(" + columnNameToLower + ");");
                            break;
                        case "datetime":
                        case "smalldatetime":
                            if (DbType == "Oracle") strclass1.AppendSpaceLine(3, "if(!PageValidate.IsDateTime(" + columnNameToLower + ")) strErr=\"" + deText + " 不是时间字符串！\\\\n\"; if(!string.IsNullOrEmpty(strErr))return strErr;");
            ...
            strclass0.DelLastComma(); // 去掉最后一个逗号
            strclass.AppendLine();
            strclass.AppendSpaceLine(2, "/// <summary>");
            strclass.AppendSpaceLine(2, "/// " + Languagelist["summaryadd"].ToString());
            strclass.AppendSpaceLine(2, "/// </summary>");
        ...
        public string CreatBLLUpdate()
        {
            ...
            #region 按参数修改  2015-09-15 张义超 不可用
            ...
            foreach (ColumnInfo field in Fieldlist)
            {
                ...
                if (field.Nullable)
                {
                    switch (CodeCommon.DbTypeToCS(columnType.Trim().ToLower()).ToLower())
                    {
                        case "int":
                        case "smallint":
                            strclass1.AppendSpaceLine(3, "if(!string.IsNullOrEmpty(" + columnNameToLower + ")&&!PageValidate.IsNumber(" + columnNameToLower + ")) strErr=\"" + deText + " 不是数字！\\\\n\"; if(!string.IsNullOrEmpty(strErr))return strErr;");
                            strclass2.AppendSpaceLine(3, "model." + columnName + "=StringHelper.Str2Int(" + columnNameToLower + ");");
                            break;
                        case "float":
                        case "numeric":
                        case "decimal":
                            strclass1.AppendSpaceLine(3, "if(!string.IsNullOrEmpty(" + columnNameToLower + ")&&!PageValidate.IsDecimal(" + columnNameToLower + ")) strErr=\"" + deText + " 不是数字！\\\\n\"; if(!string.IsNullOrEmpty(strErr))return strErr;");
                            strclass2.AppendSpaceLine(3, "model." + columnName + "=StringHelper.Str2Dec(" + columnNameToLower + ");");
                            break;
                        case "datetime":
                        case "smalldatetime":
                            strclass1.AppendSpaceLine(3, "if(!string.IsNullOrEmpty(" + columnNameToLower + ")&&!string.IsNullOrEmpty(" + columnNameToLower + ")&&!PageValidate.IsDateTime(" + columnNameToLower + ")) strErr=\"" + deText + " 不是时间字符串！\\\\n\"; if(!string.IsNullOrEmpty(strErr))return strErr;");
                            strclass2.AppendSpaceLine(3, "model." + columnName + "=StringHelper.Str2DateTime(" + columnNameToLower + ");");
                            break;
                        case "bool":
                            break;
                ...
                else
                {
                    switch (CodeCommon.DbTypeToCS(columnType.Trim().ToLower()).ToLower())
                    {
                        case "int":
                        case "smallint":
                            strclass1.AppendSpaceLine(3, "if(!PageValidate.IsNumber(" + columnNameToLower + ")) strErr=\"" + deText + " 不是数字！\\\\n\"; if(!string.IsNullOrEmpty(strErr))return strErr;");
                            strclass2.AppendSpaceLine(3, "model." + columnName + "=int.Parse(" + columnNameToLower + ");");
                            break;
                        case "float":
                        case "numeric":
                        case "decimal":
                            strclass1.AppendSpaceLine(3, "if(!PageValidate.IsDecimal(" + columnNameToLower + ")) strErr=\"" + deText + " 不是数字！\\\\n\"; if(!string.IsNullOrEmpty(strErr))return strErr;");
                            strclass2.AppendSpaceLine(3, "model." + columnName + "=decimal.Parse(" + columnNameToLower + ");");
                            break;
                        case "datetime":
                        case "smalldatetime":
                            strclass1.AppendSpaceLine(3, "if(!PageValidate.IsDateTime(" + columnNameToLower + ")) strErr=\"" + deText + " 不是时间字符串！\\\\n\"; if(!string.IsNullOrEmpty(strErr))return strErr;");
                            strclass2.AppendSpaceLine(3, "model." + columnName + "=DateTime.Parse(" + columnNameToLower + ");");
                            break;
                        case "bool":
                            break;
        ...
        public string CreatBLLDelete()
        {
            ...
            if (keyField.Trim().Length > 0)
            {
                ...
                strclass.AppendSpaceLine(2, "{");
                strclass.AppendSpaceLine(3, "return dal.DeleteList(PageValidate.SafeLongFilter(" + keyField + "list,0) );");
                strclass.AppendSpaceLine(2, "}");
            }
            #endregion
        ...
        public string CreatBLLGetModelByCache(string ModelName)
        {
            ...
            strclass.AppendSpaceLine(6, "int ModelCache =  " + NameSpace + ".Common.ConfigHelper.GetConfigInt(\"ModelCache\");");
...
*/
           ...
           // 张义超 2015-10-13
            strclass.AppendSpaceLine(3, "return dal.DataTableToList(dt);");
            strclass.AppendSpaceLine(2, "}");  
        ...
        public string CreatBLLGetListByPage()
        {
            ...
            strclass.AppendSpaceLine(2, "/// <summary>");
            //strclass.AppendSpaceLine(2, "/// " + Languagelist["summaryGetList3"].ToString()); //GetRecordCount
            strclass.AppendSpaceLine(2, "/// " + Languagelist["GetRecordCount"].ToString()); // 张义超 2014-10-21
            strclass.AppendSpaceLine(2, "/// </summary>");
            ...
            strclass.AppendLine();// 张义超 2014-10-21
            strclass.AppendSpaceLine(2, "/// <summary>");
            strclass.AppendSpaceLine(2, "/// " + Languagelist["summaryGetList3"].ToString());
            strclass.AppendSpaceLine(2, "/// </summary>");
            strclass.AppendSpaceLine(2, "public DataSet GetListByPage(string strWhere, string orderby, int pageIndex, int pageSize)");
            strclass.AppendSpaceLine(2, "{");
            strclass.AppendSpaceLine(3, "return dal.GetListByPage( strWhere,  orderby,  pageIndex,  pageSize);");
            strclass.AppendSpaceLine(2, "}");
